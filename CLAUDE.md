...
### Key Components
- **PostCard**: Circular SVG design with random positioning. Generic: `href` (default `/posts/[slug]`) and `dateLabel` props let the Work list reuse it. Interaction: on PC (`hover: hover` + `pointer: fine` only), hovering the circle rolls it to a new random spot, and it can be caught mid-roll; positions are remembered across navigation in `PostCardAnimationProvider`. The date label follows the circle but is clamped to its own width so it never overflows the card
